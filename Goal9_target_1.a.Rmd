...
```
Analizando los países que están en el top del ranking relacionados con el target
```{r}
data_rank_top %>% ggplot(aes(x=Year,y = AverageValue, color=CountryOrArea)) + geom_area()
```
...
Como podemos observar el 25% de los países tiene un valor medio por debajo de 14.32
